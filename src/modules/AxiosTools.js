...
const axiosTokenInstance = require("axios").create(
  {
    baseURL: baseURL,
    timeout: 5000,
    headers: {
    ...
    if(error.response.data.code === "token_not_valid" && error.response.status === 401 && error.response.status.statusText === "Unauthorized"){
      ...
      if(refresh_token){
        ...
        if(token_parts.exp > now){
          return axiosTokenInstance.post("/token/refresh/", {
          ...
          .then(response => {
            ...
            axiosTokenInstance.defaults.headers["Authorization"] = "JWT " + response.data.access
...
export {axiosInstance, axiosTokenInstance, baseURL}